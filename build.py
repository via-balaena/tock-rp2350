...
INTENT = {
    ...
    "tock:crc-stale-sam4l-docs": ("upstream", "The CRC documentation describes algorithms that were removed in 2021. The HIL explains a `Sam4L` naming convention for variants the enum no longer has, and the capsule still advertises SAM4L-16, SAM4L-32 and SAM4L-32C as selectable when only three identifiers are accepted. Two files, +8 -28.", "Ready, not sent. `CrcAlgorithm` holds `Crc32`, `Crc32C` and `Crc16CCITT`; `alg_from_user_int` accepts 0, 1 and 2 and answers INVAL otherwise, so SAM4L-32 and SAM4L-32C have no mapping at all and SAM4L-16 is now spelled Crc-16CCITT. The variants went in `0060488f2`, lschuermann\'s June 2021 chunked-buffer redesign, and the prose outlived them by four years. **The sharper half is the capsule contradicting itself**: the module documentation said a Crc-16CCITT result has its high bits set, giving `0xFFFFxxxx`, while the command documentation twelve lines from the code says `0x0000xxxx`. The type settles it -- `CrcOutput::Crc16CCITT` holds a u16, sam4l truncates the hardware register with `result as u16`, and the upcall encoder zero-extends -- so an application always receives `0x0000xxxx`, and the module doc was wrong about the one chip it named."),
    "tock:stm32u5-adc-justify": ("upstream", "`hil::adc` states on seven of its methods that samples are left-justified in the u16. The stm32u5xx driver returned the raw right-justified 14-bit value, so a client scaling against u16::MAX read a quarter of true scale. One file, +11 -5.", "Ready, not sent. Found by reading every chip that implements the trait against what the trait promises: six of seven comply, and this is the outlier being brought in line rather than a new convention. sam4l needs no shift because its hardware left-justifies and its own module doc says so; msp432 derives the shift from its configured resolution; the rest shift by a literal. **It reaches an application** -- `boards/nucleo_u545re_q` wires this ADC through `capsules_core::adc`, and the syscall documentation never mentions justification, so nothing downstream corrects it. The resolution is now a named constant used both by `get_resolution_bits` and by the shift, so the two cannot drift apart again. `make prepush` passed with 542 lines of real output. **Nothing ran on hardware** -- neither of us owns that board, so the scale error is an argument from the code and the register field width, not a measured voltage."),
    "tock:staticref-checked-addr": ("upstream", "`StaticRef::new` documents four obligations and can check none of them: it takes a `*const T`, and raw pointers cannot be cast to integers during const evaluation. `StaticRef::at` takes a `usize` instead, which makes the two arithmetic clauses -- aligned and non-null -- compile errors at the 364 const sites. One kernel file plus chips/rp2350 as the first caller. Nine files, +96 -28.", "Ready, not sent. The `unsafe` does not go away; it gets smaller, leaving the two clauses only a human can discharge, because only a human knows that 0x400b0000 really is that peripheral. Every one of the 331 literal base addresses in the tree is already 4-aligned or better and 240 are 4096-aligned, so adopting it breaks nothing that exists -- it constrains what can be written next. Two compile_fail doctests pin it under `ci-job-kernel`, which already runs `cargo test` in this crate, with no new dependency."),
    "tock:mmio-macro": ("upstream", "A base address is the most repeated `unsafe` in the tree: of 1,414 unsafe sites carrying no justification, 364 are a `StaticRef` built from a literal address, across 30 chip crates. `mmio!` takes the justification once per block and will not expand without it. Nine files, +229 -29.", "Ready, not sent. **Stacked, not independent**: the branch contains `747f3f24b` as well as `f4e948b02`, so it is two commits and cannot be proposed without the `StaticRef::at` change underneath it. Asking for the same claim to be written 364 times is the wrong shape; this asks once per block and removes the option of not saying it. It does not make the declaration safe -- the two uncheckable clauses are exactly what the required `safety:` argument is for."),
    "tock:capability-compile-fail": ("upstream", "Three of the four capability macros document that they cannot be called from a `#![forbid(unsafe_code)]` crate. The claim appears three times in the file and one of them had a test. Three compile_fail doctests in the shape of the existing one. One file, +36.", "Ready, not sent. Each claim was checked before a test was written for it: all three are refused, so all three hold -- they were unproven, not wrong. The mechanisms differ and the documentation now says so. **A mutation test found that two of the four tests pin less than they appear to**, and the commit message says so rather than implying otherwise."),
    "tock:elf-loadable-check": ("upstream", "Nothing downstream of the compiler is checked in Tock CI: every board is built and its `.bin` collected for size tracking, but no job asks whether the output can be loaded, and the top-level Makefile names no flashing tool anywhere. This adds one invariant -- a PT_LOAD segment must not claim file content that lands in a NOBITS section -- and a CI job that runs it over every board ELF. Two files, +210.", "Ready, not sent. **It fixes nothing**: all 34 board ELFs satisfy the invariant today, ELF32 and ELF64, ARM and RISC-V. It makes an invariant that already holds impossible to break quietly -- the same move as `#![forbid(unsafe_code)]` on a crate that already contains no unsafe. Written because of #5156: the change there was never disputed by anyone, and four days of thread went on the *explanation* the PR volunteered. A property test asserts what, claims no mechanism, and cannot be refuted by a reviewer who knows ELF better -- which is the form that dissolves that objection rather than answering it."),
    "tock:library-floor-forbid": ("upstream", "AGENTS.md states two rules about `unsafe`: `capsules/`, `chips/` and `libraries/` may not use it at all, and every use must be justified in a comment. Of the five crates under `libraries/`, one states the first where the compiler can read it and none states the second. `enum_primitive` now forbids `unsafe`; `tock-cells` confines its six sites to `map_cell`, denies the lint everywhere else, and **denies `clippy::undocumented_unsafe_blocks`, which it is the only crate in the tree able to do**. Two files, +16, attributes only.", "Ready, not sent. Level 1 of the floor plan, on top of level 0's `StaticRef::at`. Both rules already held in these crates -- nothing enforced either, which is the same gap the main audit measured at 0 against 1,226. Proven by four mutations, each rejected, including removing the `map_cell` allow and deleting one `// SAFETY:` comment, because a lint that never rejects anything has never been tested. `raspberry_pi_pico_2` builds byte-identical with and without, 262144 bytes."),
    "tock:agents-safety-comment-form": ("upstream", "`AGENTS.md` required every `unsafe` use to carry a comment starting with `### Safety`. That string appears **nowhere** in the tree. The two forms that do appear are `// SAFETY:` above a block (144) and a `# Safety` doc section on an unsafe fn (149), and they answer different questions for different readers. One file, +9 -4, prose only.", "Ready, not sent. The heading is not cosmetic: `kernel/src/lib.rs:94` denies `clippy::missing_safety_doc`, which wants `# Safety`, so the rule asked for a form the kernel's own lint does not accept. The comment above that lint records the goal of applying it across Tock, which is the direction this wording matches. The substance of the rule is carried over word for word; only the form is named, as two forms, because that is what is there. **The one `### Safety` in our own tree is ours**, written on 2026-08-29 by following the rule literally -- an agent-facing rules file that misdescribes the codebase mistrains every agent that reads it, and this is the receipt."),
    "tock:uart-size-contract": ("upstream", "`hil::uart` documents `Err(SIZE)` when a length is larger than the passed slice, in three places -- `transmit_buffer`, `receive_buffer` and `receive_automatic`. `UartDevice::transmit_buffer` checked only for zero, while `receive_buffer` in the same file checks the slice -- so nothing on the transmit path bounded the length. `chips/segger` then indexes the caller's slice with it, in both directions. Three files, three commits. Third commit added 2026-09-13: sam4l's `receive_automatic` clamped with `cmp::min` where the trait says `Err(SIZE)`, and **the trait's two implementers diverge in opposite directions** -- lowrisc answers SIZE and skips busy, sam4l checks busy and skips SIZE.", "Ready, not sent. The segger half is the sharp one: `transmit_buffer` copies `tx_data[i]` for every `i` in `0..tx_len` and the receive path indexes the buffer while a cursor is below `rx_len`, so a length past the end indexes out of bounds, and Tock does not allow unwinding panics. 23 of the 26 implementations already answer `Err(SIZE)`. Found by auditing what the trait states against every implementation -- see [[tock-uart-contract-audit]]. Merges clean with rp2-uart-abort-fix, which touches a different part of the same mux file."),
    "tock:uart-receive-busy": ("upstream", "`hil::uart` documents `Err(BUSY)` for `receive_buffer` when a reception is already outstanding. Six drivers did not check, and each stores the new buffer over the old one, so the previous `&'static mut [u8]` is dropped and its owner never gets a callback. Every one of those files already answers `Err(BUSY)` on its transmit path. Six files, +66 -13.", "Ready, not sent. **Two commits on purpose**: five drivers cost nothing, and sam4l costs 512 bytes of text on imix (169004 to 169516, 0.30%), so the cost can be weighed on its own rather than averaged into a series that is otherwise free. sam4l's own `receive_automatic` already guards this exact case with `usart_rx_state != Idle`, and the shape is copied from it. **Latent, not live** -- `MuxUart` serialises, so no in-tree caller reaches the window. Conflicts with `uart-size-contract` in one hunk of `chips/segger/src/rtt.rs`, where both add a guard to the same method and both are wanted; clean against `rp2-uart-abort-fix`."),
    "tock:lpc55s6x-abort-callback": ("upstream", "`hil::uart` says a cancelled transfer MUST make a callback returning the buffer, and defines `Err(BUSY)` from an abort as *cancelled successfully; there will be an appropriate callback*. Both aborts in `chips/lpc55s6x/src/uart.rs` returned `Err(BUSY)` and no callback could follow: each disabled its interrupt and set a state that is **declared, set, and read nowhere**. One file, +32 -4.", "Ready, not sent. **The only LIVE bug the hil::uart audit found.** The buffer is stranded and the state never returns to `Idle`, so every later operation on that direction answers BUSY for the life of the board. lpc55s69-evk runs three clients on one `MuxUart`, and `start_receive` calls `receive_abort` routinely to restart a read -- its own comment says it aborts so the `received_buffer` handler can recalculate the length, and that handler was never reached. The fix completes the transfer inline, in the same order `handle_interrupt` already uses, which is also how sam4l's `abort_tx` resolves it. **Not tested on hardware**: no such board here."),
    "tock:bench/uart-abort-pico2w": ("never", "The vehicle for the 2026-09-13 silicon A/B: `pico2w-typed` with `rp2-uart-abort-fix` merged and nothing else, so the only difference on the RP2350 path is where `rx_status` returns to Idle. Both kernels build to text=323116.", "Not a contribution -- the board only exists on our branches, so there is nowhere upstream the two meet. Kept so the measurement can be repeated rather than only cited, the same reason `bench/uart-fix-at-pinned-rev` is kept. Transcripts at `learning/bench/uart-abort-silicon/`."),
    "tock:main": ("never", "**The trunk.** Renamed from `rp2350-distro` and made the fork's default branch on 2026-09-13, so landing on the repository shows our work rather than upstream's. `master` stays pinned to upstream as the clean reference that rebases and A/B baselines are cut from. Our own flavour of Tock for the RP2350, and the branch all work lands on. **Head `fab074b7c`, 76 commits ahead of upstream after all ten of 2026-09-13's branches merged in.** Kept current with a merge rather than a rebase so the hand-resolved conflicts survive.", "Not a staging area and not intended for upstream as a branch; pull requests are cut from the feature branches instead. Merged current with upstream on 2026-09-12, after which all six RP2 boards build and the 19 PIO tests pass. One of its eleven merges needed manual three-way resolution -- git merges the moved PIO driver clean and silently reverts five fixes and deletes the test module -- which is why this is merged rather than rebased."),
    ...
    "tock:rp2-local-board": ("upstream", "Replaces the ELF splice on the RP2 boards with a flash image built by `tockloader local-board`: `make init` records the board, `tockloader install --local-board` adds applications from `.tab` archives, and `make flash` hands the flat image to picotool, openocd or probe-rs. `make` never takes an APP, which is the shape esp32-c3-devkitM-1 already uses. elf2uf2-rs leaves the tree. Two commits, nine files, +159 -126.", "**Open as #5160**, head `41e7c92ee`, reworked to the workflow bradjc asked for on the thread and rebased onto master once #5158 merged. This is the successor bradjc asked for on #5156 and the answer given there. It supersedes that PR's `objcopy -R .stack` on the boards it touches, and is based on master rather than on that branch, so either can merge first. #5158 merged first and changed the same two README lines; the rebase resolved both by keeping this change's replacement, and #5158's ADC commit is untouched. A fifth board rides along: raspberry_pi_pico_w includes raspberry_pi_pico's Makefile and its `prog` is at 0x10090000, so it sets APP_ADDRESS for itself. The Pico 2 half ran on silicon over all three load paths -- SWD, the bootloader and probe-rs -- and placed two applications at once, which the splice route cannot do at all; **no RP2040 was ever booted** -- there is no such board here and mainline QEMU has no machine for it."),
    ...
    "tock:rp2-uart-abort-fix": ("upstream", "The abort ordering in **six** chip drivers plus the two buffer-ownership bugs in the mux. `handle_deferred_call` calls the client back before returning the driver to Idle, and `receive_buffer` refuses unless it is Idle, so the mux's restart from inside that callback is always refused -- and the mux answers a failed restart by ending every client's receive. Three commits, seven files, +90 -19.", "Ready, not sent. **Extended from three drivers to six on 2026-09-13**, head `660b708e0`: stm32f303xc, stm32f4xx and stm32wle5xx carry the identical block and were missing. They were found by auditing what `hil::uart` states against all 26 implementations, not by reading those files -- see [[tock-uart-contract-audit]]. stm32f4xx alone reaches five boards, so the three added drivers cover seven. **VERIFIED ON RP2350 SILICON 2026-09-13**, which the QEMU run could not reach: on a Pico 2 W over SWD, the unfixed kernel answers the application `read -> 0 bytes, Err(BUSY)` and the process console then ignores `help` entirely -- no echo, no prompt -- while the fixed kernel leaves the read outstanding and completes it `read -> 16 bytes, Ok(())` with the input echoed. **The second half, that the process console dies with it, had never been measured before**; the earlier QEMU A/B on the pinned hifive1 kernel covered only the application read. Transcripts at `learning/bench/uart-abort-silicon/`. Only rp2040/rp2350 were exercised; the other four drivers take the same edit and no board for them is here."),
    ...
    "libtock-rs:stepper": ("upstream", "The userspace half of the stepper: `libtock_stepper` and an example that turns a revolution each way.", "**The kernel half landed in `main` on 2026-09-13**, so this no longer waits on it. It remains the only half still on its own branch."),
    "libtock-rs:screen-completion": ("upstream", "Three ways `apis/display/screen` disagreed with the capsule about whether a command completes synchronously or in an upcall. All ten completing operations discarded the upcall's status argument, so an operation that failed inside the capsule reported `Ok(())`. `get_rotation` subscribed and waited for an upcall that is never scheduled. `set_power` and `set_invert` returned on the command's own answer for commands the capsule enqueues, which is acceptance reported as completion. Three files: the API, its tests, and the screen fake.", "The libtock-rs session's branch and Jon's description, cut off master at b7f1759. Facts relayed by them; the capsule side checked here against `capsules/extra/src/screen/screen.rs` -- command 21 is a bare `CommandReturn::success_u32` at :587 with no enqueue, while 2 and 6 are `enqueue_command` at :534 and :542, so the wrapper was wrong in both directions at once. **The fake was wrong in both directions too, which is why the tests passed**: it scheduled an upcall for 21 that the kernel does not send, and none for 2 and 6 that it does, so a test asserted a value on a call that could only hang. A fake written from the caller's model cannot disagree with the caller. **Independent of our capsule changes** -- they report the three files are byte-identical between master and the fix's parent, so it cherry-picks unadapted, and none of the three defects depends on the queue-on-BUSY behaviour. Their commit message on our main cites a fork SHA when explaining the remaining meaning of BUSY; they are rewording it, because a SHA in this fork means nothing to a reviewer. **The same applies to our own screen commits if they ever go up**, which cite each other by fork SHA in both directions."),
    ...
    "libtock-rs:main": ("never", "**The trunk**, and the fork's default branch since 2026-09-14 -- the same shape `tock:main` has. Built from `master` and then merged with `doom/frame-probe`, `async/alarm` and `bench/reclaim-leak`, so every 2026 commit written here is reachable from one branch. `master` stays pinned to upstream as the reference the PR-shaped branches are cut from.", "Not a staging area and not intended for upstream as a branch; the five one-concern branches beside it are what a pull request would be cut from, and they are kept precisely because a minimal diff is the work product and re-extracting one costs hours. Merged rather than rebased. **Building it found three latent failures** that `doom/frame-probe` carries on its own: `kit_wad_xip` embedding a gitignored 1.8 MB generated WAD, `kit_doom_frame` overflowing nrf52's 62K by 67584 bytes, and `doom.rs` never having passed rustfmt. All three are fixed here and `make test` passes."),
    "libtock-rs:doom/frame-probe": ("never", "Doom on the Pico 2 W, playable at 9 fps on the kit's ST7796 -- the port, the WAD converter, and the frame-path probes. The C is GPL-2.0-only and lives in `via-balaena/doomgeneric`; nothing GPL is vendored here.", "Never upstream as a branch. The Rust is original and stays Apache-2.0 OR MIT; `--features doom` is what links GPL-2.0-only code, and `LICENSE-DOOM` records why a binary built that way may only be distributed under GPL-2.0-only. **Two pieces of it are general and did land in main regardless of Doom**: the platform table's Pico 2 W row with 480K of app RAM, and `--trailing-padding` in the runner, which lifted the usable app flash from 2 MiB to 3.5 MB by dropping a power-of-two rule ARMv8-M does not have."),
...
WORK = {
    ...
    "rp2350: build the flashed image with tockloader, not by splicing an ELF": {
        "short": "Flash image from tockloader, not a splice",
        "verify": "silicon",
        "note": "PR #5160, first commit. Removes the conditions for #4770 rather than patching it: no ELF is opened, so no program header is recomputed. `program` and `program-openocd` give way to esp32-c3-devkitM-1's `make init` / `tockloader install --local-board` / `make flash`, so there is no APP variable at all. Run on a Pico 2 W every way the rules offer -- SWD, the bootloader and probe-rs -- and it places two applications at once, which the splice cannot do at all.",
    },
    "rp2040: the same image route, and one UF2 converter instead of two": {
        "short": "Same route on RP2040, picotool only",
        "verify": "build",
        "note": "PR #5160, second commit. elf2uf2-rs cannot read a flat image, so these four boards move to picotool and elf2uf2-rs leaves the tree entirely -- it has no callers left. Measured rather than assumed: from the same kernel ELF the two converters agree on all 102,400 addresses with zero disagreements. raspberry_pi_pico_w is the fourth board: it includes raspberry_pi_pico's Makefile and needed its own APP_ADDRESS, 0x10090000. No RP2040 was booted -- there is no such board here and mainline QEMU has no machine for one.",
    },
...
FACTS = {
    "tock:crc-stale-sam4l-docs": [
        ("What the HIL promises that it does not have", "`kernel/src/hil/crc.rs` tells the reader that algorithms prefixed with `Sam4L` are native to that chip and need no software post-processing. No variant carries that prefix. The enum holds `Crc32`, `Crc32C` and `Crc16CCITT`, and has since `0060488f2` in June 2021 replaced `CrcAlg`, whose `Sam4L16`, `Sam4L32` and `Sam4L32C` the sentence was written for. `Sam4L16` became `Crc16CCITT`, keeping the polynomial and the absence of post-processing; the other two were dropped."),
        ("Three algorithms a caller cannot select", "The capsule's module documentation lists five under a heading that says the capsule supports them: Crc-32, Crc-32C, SAM4L-16, SAM4L-32 and SAM4L-32C. `alg_from_user_int` accepts 0, 1 and 2 and returns `None` for everything else, so command 1 answers INVAL. Two of the five names map to nothing at all."),
        ("The capsule contradicts itself, and the type settles it", "The module documentation said the sixteen-bit result sits in the low bits with the high bits set, so values are of the form `0xFFFFxxxx`. The command documentation, twelve lines above the code, says `0x0000xxxx`. `CrcOutput::Crc16CCITT` holds a `u16`; `chips/sam4l/src/crccu.rs:215` truncates the hardware register with `result as u16`, discarding exactly the bits the module doc promised; and `encode_upcall_crc_output` zero-extends. An application always receives `0x0000xxxx` -- on the SAM4L, which is the chip the paragraph was about."),
        ("What was run", "`cargo doc --no-deps` on both touched crates, exit 0 with no unresolved intra-doc links; `cargo fmt --check`; Tock's own license checker; and `make -C boards/imix`, which is SAM4L and wires this capsule. Every changed line was asserted to be a doc comment, with a control confirming the filter catches a code line. Nothing executable changed."),
    ],
    "tock:stm32u5-adc-justify": [
        ("The promise, and who keeps it", "`hil::adc` states on seven separate methods that all samples are the raw value left-justified in the u16. Seven chip crates implement it. msp432 shifts by `8 - 2 * resolution`, deriving the alignment generically; nrf52 shifts by four with the comment \"shift left to meet the ADC HIL requirement\"; rp2040, stm32f303xc and stm32f4xx shift by four; sam4l applies no shift because its hardware left-justifies, which its own module doc lists alongside the reference voltage and the gain. stm32u5xx returns `data as u16`."),
        ("What a client reads instead", "`get_resolution_bits` returns 14 and there is no alignment configuration anywhere in the file -- the STM32U5 ADC has no data-alignment bit. Samples span `0x0000`-`0x3FFF` where the contract promises `0x0000`-`0xFFFC`, so a client that trusts the HIL and scales against `u16::MAX` reads a quarter of true scale. The driver said so itself: the line above the call read \"The data we provide to the client is the raw value directly\"."),
        ("It reaches an application", "`boards/nucleo_u545re_q` builds an `AdcMuxComponent` over `stm32u545::adc::Adc`, which `stm32u545/src/lib.rs:8` re-exports straight from stm32u5xx, and answers `capsules_core::adc::DRIVER_NUM`. `doc/syscalls/00005_adc.md` never mentions justification, so nothing downstream corrects it. This is the one thing separating the change from a style fix."),
        ("The shape of the fix", "The resolution becomes a named constant used both by `get_resolution_bits` and by the shift, so the reported resolution and the justification cannot drift apart again -- which is how the defect existed in the first place, with the two sitting in separate places and nothing connecting them."),
        ("What was run, and what was not", "`make prepush` passed with 542 lines of real output rather than a cached run, exit 0, zero error or warning lines; `make -C boards/nucleo_u545re_q` builds. The arithmetic was computed rather than reasoned about: `0x3FFF` maps to `0xFFFC`. There is exactly one `sample_ready` call site in the driver, so no second path still returns unshifted values. **No hardware.** Neither of us owns that board; the scale error is an argument from the code and the register field width, not a measured voltage. No test was added -- no chip crate has register-level ADC tests and there is no fake register block to write one against."),
    ],
    "tock:staticref-checked-addr": [
        ("Why the existing constructor cannot check anything", "`StaticRef::new` documents four obligations -- aligned, non-null, dereferencable as `T`, and valid for the program duration -- and takes a `*const T`. Raw pointers cannot be cast to integers during const evaluation, so not one of the four can be tested there. All four are left to the caller, at 389 sites."),
        ("What changes, and where it bites", "`StaticRef::at` takes the address as a `usize`, which makes the two arithmetic clauses checkable. In a `const` or `static` binding -- 364 of those sites -- a misaligned or null address becomes a compile error naming the offending line rather than a fault at boot: `error[E0080]: evaluation panicked: StaticRef: base address is not aligned for this register type`."),
        ("Null was already rejected, and the assertion is kept anyway", "`NonNull`'s niche refuses zero in const context on its own, with `encountered 0, but expected something greater or equal to 1` -- so that half was never unprotected in a const binding. The explicit assertion is kept for the clearer message, and because it also covers the 25 sites that build a `StaticRef` at runtime, where the existing `new()` would be undefined behaviour rather than an error."),
        ("Adopting it breaks nothing that exists", "Every one of the 331 literal base addresses in the tree is already 4-aligned or better, and 240 of them are 4096-aligned. The constraint is on what can be written next, not on what is already there -- which is why it can go in ahead of the migration rather than behind it."),
        ("What was run", "Two compile_fail doctests, **both confirmed to fail when the assertion is removed** -- a test whose precondition is the fix cannot validate the fix. They run under `ci-job-kernel`, which already runs `cargo test` in this crate, so there is no new dependency and no new CI job. Built byte-identical before rustfmt reflowed the shortened declarations; afterwards 21 bytes differ and all of them are line numbers inside `core::panic::Location` records in the reformatted files. No instruction changed; `.text` and `.storage` are unmoved at 65112 and 3460."),
    ],
    "tock:mmio-macro": [
        ("The number that decides the shape", "Of 1,414 `unsafe` sites in the tree carrying no justification, 364 -- a quarter -- are a `StaticRef` built from a literal address, spread over 30 chip crates. Every one of them makes the same claim. A lint demanding 364 individual justifications would be asking for the same sentence 364 times, which is how a rule teaches people to write filler; `mmio!` asks once per block."),
        ("The argument is required, and the failure says so", "A missing `safety:` is a `compile_error!` that shows the correct form, rather than the macro's default `no rules expected this token`. Alignment and non-null still come from `StaticRef::at`, so a misaligned address inside an `mmio!` block fails to compile too -- the two constraints compose instead of replacing each other."),
        ("What it deliberately does not do", "It does not make the declaration safe. Two clauses of `StaticRef::at`'s contract cannot be checked by anything -- that the address really is the block named, and that it outlives the program -- and those are exactly what the argument is for. What the macro removes is the option of not saying."),
        ("The migration, measured", "chips/rp2350: 14 declarations across 8 files become 8 blocks with 8 justifications. Direct `StaticRef` constructions in that crate go from 14 to zero; the `unsafe` that remains is a chip constructor, a panic path and two vector tables, none of it MMIO. The other 29 chip crates are mechanical and can follow separately."),
        ("It is stacked", "The branch carries `747f3f24b` as well as `f4e948b02`. The macro is built on `StaticRef::at` and cannot be proposed independently of it -- and cross-fork stacked pull requests do not work on GitHub, so this is one submission of two commits or it waits."),
        ("What was run", "Six doctests, four of them compile_fail, covering both refusals, under `ci-job-kernel` with no new dependency. All five RP2 boards build. `raspberry_pi_pico_2` is unchanged at text 69164; against the parent commit 21 bytes differ and every one is a line number inside a `core::panic::Location` record, with the filename pointer and length identical either side. No instruction changed."),
    ],
    "tock:capability-compile-fail": [
        ("A claim written three times with one test", "Three of the four capability macros document that they cannot be called from `#![forbid(unsafe_code)]` crates. `create_capability` had a compile_fail doctest pinning it. `create_typed_capability`, `define_capability_type` and `mint_defined_capability` did not -- the same sentence, three times, unverified."),
        ("Each claim was checked before a test was written for it", "The forbidden call was compiled as an ordinary doctest first, to see whether it really is refused. All three are refused, so all three claims hold: they were unproven, not wrong. Writing the compile_fail test first would have proved only that something failed, not that the documented thing did."),
        ("The mechanisms differ, and the documentation now says so", "`create_typed_capability` and `define_capability_type` carry an internal `allow(unsafe_code)` that collides with a forbid, giving `error[E0453]: allow(unsafe_code) incompatible with previous forbid`. `mint_defined_capability` carries no such allow, and its expansion contains an `unsafe` block that a forbid rejects on its own. All three refusals were confirmed by compiling the forbidden call as an ordinary doctest. **Which of the two gates does the refusing, per macro, was not isolated and is not claimed here** -- it matters because only the allow-collision kind survives someone deleting the internal allow."),
        ("What the mutation test established, including about the test that was already there", "Against the full regression these exist to catch -- the capability traits made safe, the unsafe impls made safe, and the `allow(unsafe_code)` removed -- `create_capability` and `define_capability_type` fail, while `create_typed_capability` and `mint_defined_capability` do not. The two that survive have a second, independent gate: their expansions still contain an `unsafe fn` or `unsafe` block that forbid rejects on its own, so those two tests pin the outer gate only. **Which gate does what per macro is not isolated here and is not claimed.** An earlier, incomplete mutation -- removing `unsafe` while leaving `allow(unsafe_code)` in place -- still tripped E0453, and made all four tests look like they had fired."),
        ("What was run", "All four doctests run under `ci-job-kernel`, which already runs `cargo test` in the kernel crate, so there is no new dependency and no new CI job. One file, +36 lines, nothing executable changed."),
    ],
    "tock:elf-loadable-check": [
        ("The invariant, and why a tool cares", "A PT_LOAD segment's `p_filesz` is the count of bytes a loader copies out of the file. If part of that span lands in a NOBITS section, the file is asking the loader to copy bytes that do not exist. elf2uf2-rs refuses such a file with `ELF contains memory contents for uninitialized memory at 20000000` -- and 0x20000000 is exactly the `p_vaddr` the check reports."),
        ("It fixes nothing, and that is the point", "All 34 linked board ELFs in the tree satisfy it: 33 ELF32 and 1 ELF64, ARM and RISC-V. The invariant already holds everywhere. Giving it to CI is the same move as putting `#![forbid(unsafe_code)]` on a crate that already contains no unsafe -- the 8 crates that did that measure exactly 0 while the prose-only layers carry 1,226. A constraint is cheapest to add while it is already satisfied."),
        ("The gap it fills", "`ci-job-compilation` builds every board; `ci-job-collect-artifacts` copies `.bin` files for size tracking. No job inspects an artifact for loadability. Measured: the top-level Makefile contains zero references to `program`, `flash`, `uf2`, `elf2uf2` or `picotool`, and neither do the workflow files. The flashing path is not exercised anywhere in CI, which is why #4770 sat from April with no answer -- nothing could see it."),
        ("It asserts a property and claims no mechanism", "The output names the segment, the section, and the byte count. It says nothing about linkers, objcopy, or BFD, and nothing about how a file came to be that way. This is deliberate: on #5156 the 13-line change was never disputed -- bradjc *\"if this improves things then fine\"*, ppannuto *\"a reasonable temporary patch\"*, lschuermann *\"I'm fine fixing this issue\"* -- and the whole dispute was over a volunteered causal story that nothing depended on and that had already been refuted once."),
        ("Two ways it refuses to pass quietly", "Finding no ELF is an error, not a clean run; so is every input being unreadable. A checker for a defect whose entire character is that it fails silently must not be able to fail silently itself. Both paths were exercised: an empty directory exits non-zero, and so does a file that is not an ELF."),
        ("What was run", "Proven both directions. It clears all 34 board ELFs. It fires on `raspberry_pi_pico_2-app.elf` -- what `make program` actually produces -- reporting 0x3000 bytes at 0x20000000 in `.stack`, and clears on the same file after `arm-none-eabi-objcopy -R .stack`, which is #5156's fix. Tock's own license checker passes. The Makefile wiring was verified from make's parsed rule database rather than by running it, because `make -n` executes recursive sub-makes and could not answer the question. No external dependency: the ELF headers are read with `struct`."),
    ],
    "tock:library-floor-forbid": [
        ("The rule, and who keeps it", "`AGENTS.md:34` says new code in `capsules/`, `chips/` and `libraries/` may not use `unsafe` *at all*. Measured across the five crates under `libraries/`: `enum_primitive` 0 in-code uses, `tock-tbf` 0, `tock-cells` 6, `riscv-csr` 5, `tickv` 22. Exactly one of the five, `tock-tbf`, carries `#![forbid(unsafe_code)]`, with a one-line note saying why it can. The other four state the rule nowhere the compiler can read it."),
        ("enum_primitive: free, because it already complied", "503 lines across `libraries/enum_primitive/src/lib.rs` and `cast.rs`, casting between integers and enums, with zero `unsafe`. `capsules/core` and six boards depend on it. Adding the forbid changes no behaviour and costs no bytes -- it removes the option of the rule quietly stopping being true, which is the cheapest moment to spend on a constraint."),
        ("tock-cells: why deny and not forbid", "`map_cell` needs `unsafe` to hand out a reference into a `MaybeUninit` it has just proved is initialized, and `forbid` admits no inner `allow` -- that collision is `error[E0453]`. So the choice in this crate is `deny` or no lint at all. All six sites sit in that one module; `take_cell`, `optional_cell` and `numeric_cell_ext` have none between them, and those are the cell types most of the tree uses. The crate denies the lint and allows it back for `map_cell` alone."),
        ("The second rule, and the only crate that can state it", "AGENTS.md also requires every `unsafe` use to be justified. All six of `map_cell`'s blocks already carry a `// SAFETY:` comment, so `#![deny(clippy::undocumented_unsafe_blocks)]` costs nothing today and keeps it true. Counted over the tree, **every other crate with unsafe blocks has at least one without a comment** -- `kernel` is at 120 of 188, `sam4l` 0 of 43, `stm32u5xx` 0 of 49, `x86` 0 of 67. This is the block half of what `kernel/src/lib.rs:94` already denies for doc comments via `clippy::missing_safety_doc`, whose own comment records the goal of applying it across Tock."),
        ("Four mutations, each rejected", "An `unsafe` block appended to `take_cell.rs` fails with `error: usage of an unsafe block`. The same block in `enum_primitive/src/cast.rs` fails. Removing `#[allow(unsafe_code)]` from the `map_cell` declaration fails, which shows the `deny` is live rather than inert. Deleting a single `// SAFETY:` comment fails with `error: unsafe block missing a safety comment`. Each was reverted and the crates rebuilt clean afterwards."),
        ("What was run", "`make ci-job-libraries` exits 0, covering the four library crates it names and tock-cells' 32 tests. `cargo clippy -- -D warnings` is clean, and `tock-cells` is one of the 137 workspace packages, so the top-level clippy job reaches it -- a lint CI never runs would be decoration. Plain `cargo build` emits no unknown-lint warning for the `clippy::` attribute. `make -C boards/raspberry_pi_pico_2` exits 0 and the binary is byte-identical with and without the change, 262144 bytes, compared by stashing and rebuilding rather than by reasoning that attributes cannot emit code. `cargo fmt --check` and Tock's license checker are clean."),
        ("What it does not do", "`riscv-csr` and `tickv` keep 5 and 22 `unsafe` sites and are untouched here; whether their uses can be confined to one module the way `map_cell`'s are was not examined. Nothing was proposed upstream."),
    ],
    "tock:agents-safety-comment-form": [
        ("What the rule asked for, and what is there", "`AGENTS.md` said all `unsafe` usage must carry a comment starting with `### Safety`. Counted over `upstream/master` at `0cc36ca6c`: `### Safety` **0**, `// SAFETY:` **144**, a `#`-level `Safety` doc section **149**. A positive control confirms the searches can see -- 589 files contain the word `unsafe` -- and a negative control for `##### Safety` returns 0."),
        ("They are two forms because they answer two questions", "A `# Safety` section on a `pub unsafe fn` tells a *caller* what they must guarantee before calling. A `// SAFETY:` comment above an `unsafe` block tells a *reader* why that particular operation is sound. Rust separates them and so does Tock; one rule covering both is why it was hard to follow."),
        ("The heading was not cosmetic", "`kernel/src/lib.rs:94` carries `#![deny(clippy::missing_safety_doc)]`, and that lint wants a `# Safety` heading. The rule as written asked for a form the kernel's own lint does not accept. The comment two lines above it says the goal is to apply the lint across all of Tock, starting with the kernel, so the new wording points the same way rather than against it."),
        ("The only instance in our tree is ours", "`boards/raspberry_pi_pico_2/src/lib.rs:221`, commit `5fcc2db00`, 2026-08-29 -- written by following `AGENTS.md` literally. **It does not exist upstream**; that file is ours. So the rule has produced exactly one instance anywhere, on a branch that has not been proposed."),
        ("Measured on the right tree, on the second attempt", "The first counts came out 133 / 141 / 1 because they were run in a checkout sitting on `learning/series`, which carries our own branches and is behind upstream. Re-run against `upstream/master` they are 144 / 149 / 0, and the kernel lint that decides the whole argument is **not present** on the older tree at all. A count is a fact about the tree it was taken on."),
        ("What was run, and what changed", "One file, +9 -4, prose only. Tock's license checker passes. Nothing in CI reads `AGENTS.md`, so there is no gate to satisfy and none was claimed. The substance is carried over word for word -- discuss exactly why the unsafe code is necessary and what checks are needed and completed -- and only the form is renamed."),
    ],
    "tock:rp2-uart-abort-fix": [
        ("What the contract says, and what these drivers do", "`hil::uart`'s client documentation states that *a call to `receive_buffer` made within this callback SHOULD NOT return `Err(BUSY)`; when this callback is made the UART should be ready to receive another call*. In six drivers `handle_deferred_call` invokes the client and only afterwards sets the state to Idle, while `receive_buffer` and `transmit_buffer` refuse unless the state is already Idle."),
        ("Why it reaches a user", "`MuxUart`'s `received_buffer` restarts the underlying read from inside the callback whenever any client still has one outstanding, so the restart is always refused, and the mux answers a failed restart by ending **every** client's receive rather than the one that failed. On a board pairing a process console with the userspace console capsule on one mux, an application's first console read is accepted and then killed, and the process console stops receiving at the same moment."),
        ("Six drivers, found in two passes", "rp2040, rp2350 and sifive hold the block character for character and were fixed first. stm32f303xc and stm32wle5xx hold the same block with their own state enum; stm32f4xx holds it with the abort reason carried inside the state variant, and that reason is bound out of a copy before the state is touched, so hoisting the reset past it is safe. The second three were found by auditing the trait's stated guarantees across all 26 implementations -- a scan that rediscovered the first three without being told about them, which is what made its other candidates worth reading."),
        ("Size, stated separately because it differs", "The first three build byte-identical -- text, data and bss unchanged on hifive1, raspberry_pi_pico and nano_rp2040_connect, images matching to the byte. The three stm32 drivers do **not**: text, data and bss are unchanged (94252/0/18740 on stm32f3discovery, 96300/0/19768 on nucleo_f429zi, 75820/0/17492 on lora_e5_mini, identical with and without) but the instruction bytes move, because the reordered statement is scheduled differently. **A control established the build is otherwise reproducible** -- two builds of unchanged source are byte-identical -- so the difference is the change and not the toolchain."),
        ("Reach", "stm32f4xx alone is used by five boards, so the three added drivers cover seven: stm32f3discovery, nucleo_f429zi, stm32f429idiscovery, stm32f412gdiscovery, weact_f401ccu6, nucleo_f446re and lora_e5_mini. Built clean: raspberry_pi_pico, hifive1, stm32f3discovery, nucleo_f429zi, lora_e5_mini, stm32f412gdiscovery and weact_f401ccu6. `cargo fmt --check` and Tock's license checker pass."),
        ("What was run, and what was not", "The behaviour itself was A/B'd under QEMU on the pinned hifive1 kernel: `Err(BUSY)` without the change, the read left outstanding with it, and 16 bytes typed completing it `Ok`. **None of the seven stm32 boards was run** -- none is owned here, so for those the argument is that the code is the same block, not a measurement."),
    ],
    "tock:uart-size-contract": [
        ("An asymmetry inside one file", "`hil::uart` states `Err(SIZE)` for both directions -- *`tx_len` is larger than the passed slice* and *`rx_len` is larger than the passed slice*. `UartDevice::receive_buffer` checks `rx_len` against the slice; `UartDevice::transmit_buffer` checked only `tx_len == 0`. The two sit in the same file."),
        ("Nothing else on the transmit path checks", "`virtual_uart.rs:429` stores the buffer and the length and hands both down without comparing them, so an oversized transmit length reached the driver as given. The receive path is bounded at `virtual_uart.rs:472`, which is what shields the receive-side divergences found in the same audit and leaves the transmit-side ones exposed."),
        ("What segger does with it", "`chips/segger/src/rtt.rs` copies `tx_data[i]` for every `i` in `0..tx_len`, and its receive path advances a cursor while it is below `rx_len`, using that cursor to index the receive buffer. Neither compares the length against the slice -- the copy is at `rtt.rs:246` and the cursor loop at `rtt.rs:313` -- so a length past the end indexes out of bounds. The driver backs the nRF52840-DK configurations and lpc55s69-evk."),
        ("The fix is what the trait already says", "`Err(SIZE)` in both methods, which 23 of the 26 implementations in the tree already answer. Nothing new is invented and no behaviour changes for a caller that passes a length within its slice."),
        ("What was run", "`make ci-job-capsules` exits 0. `cargo clippy -D warnings` on capsules-core and segger, `cargo fmt --check`, and Tock's license checker are clean. nrf52840dk, lpc55s69-evk and raspberry_pi_pico build, and text, data and bss are unchanged on the two segger boards, measured by stashing the diff and rebuilding. The branch merges clean with rp2-uart-abort-fix, checked with `git merge-tree`."),
        ("receive_automatic: two implementers, one guarantee each", "`ReceiveAdvanced::receive_automatic` lists both `Err(SIZE)` and `Err(BUSY)`. `chips/lowrisc/src/uart.rs` answers SIZE and never checks busy; `chips/sam4l/src/usart.rs` checks busy and clamps with `cmp::min` instead of answering SIZE. **No caller is affected** -- the only in-tree caller, `capsules/extra/src/nrf51822_serialization.rs`, refuses an oversized length itself before calling down or passes `buffer.len()` exactly, which is the same shape as the mux bounding `rx_len` for a driver that will not."),
        ("The sam4l 512 bytes is a step, not a per-check cost", "The SIZE guard costs 512 bytes of text on imix, 169004 to 169516. The busy guard on sam4l's `receive_buffer`, on the `uart-receive-busy` branch, costs the same 512. **Applying both together also gives 169516**, measured by patching one branch\'s sam4l diff onto the other and rebuilding. Whichever lands first pays it and the other is free."),
        ("No test, and why", "A host test would have to construct the mux, whose constructor takes a `&'static mut [u8]`, and the one existing test in `capsules/core` (`virtual_alarm`) uses stack locals only. Supplying that buffer means bringing an allocator into a crate that is `no_std` and `forbid(unsafe_code)` under `cfg(test)` -- a decision for the maintainers rather than something to add beside a bounds check. **The behaviour was not executed**: the defect is read from the indexing, not reproduced on a board, and no segger hardware is owned here."),
    ],
    "tock:uart-receive-busy": [
        ("What the trait says and what these drivers did", "`hil::uart` lists `Err(BUSY)` for `receive_buffer`: *the UART is already receiving and has not made a reception complete callback yet*. apollo3, esp32, lowrisc and qemu_virt_chip each call `self.rx_buffer.replace(buf)`, segger calls `put`, and `TakeCell::replace` returns the previous value, which is discarded. sam4l instead starts a second DMA transfer over the first. In every case the earlier buffer is lost and the client that owned it is never called back."),
        ("The argument is inside each file already", "Every one of the six answers `Err(BUSY)` on its transmit path in exactly this situation, and the guard added to each is that file's own transmit check applied to receive. sam4l is stronger still: `ReceiveAdvanced::receive_automatic`, further down the same file, already guards with `usart_rx_state != Idle`, so `receive_buffer` is being made to match its sibling."),
        ("Latent, and that is stated rather than implied", "`MuxUart` serialises. `UartDevice::receive_buffer` refuses a second receive at `virtual_uart.rs:471`, and `start_receive` only calls down when it holds the buffer, so **no in-tree caller reaches the window today**. It remains what the HIL specifies, and the drivers are usable outside the mux. **Nothing was run on hardware** -- none of these boards is owned here."),
        ("Split by cost, deliberately", "Five drivers are free: text unchanged on redboard_artemis_nano, redboard_artemis_atp, esp32-c3-devkitM-1, earlgrey-cw310, qemu_rv32_virt, nrf52840dk and lpc55s69-evk. sam4l costs **512 bytes on imix**, 169004 to 169516, 0.30%, with data and bss unchanged. That is a separate commit so a reviewer can take the free five and weigh sam4l on its own. The figure is deterministic: two builds of unchanged source give the same number, and stashing the change restores the baseline."),
        ("A measurement bug caught in the harness, not the code", "The first build sweep printed `rc=0` for eight boards including one whose path did not exist. In `printf ... \"$(basename $b)\" \"$?\"` the command substitution expands first and clobbers `$?`, so every status printed was `basename`'s. The apollo3 boards live under `boards/apollo3/`, and the run that appeared to build them had built nothing. Re-measured with the status read into a variable before anything else ran."),
    ],
    "tock:lpc55s6x-abort-callback": [
        ("A state that is written and never read", "`AbortRequested` is declared at `uart.rs:546` and `:553`, set at `:945` and `:994`, and read at no line in the file. The driver has no `DeferredCall`, and both client callbacks are made from `handle_interrupt` (`:743`, `:768`), which each abort disables immediately before setting that state. Control: rp2350 names `AbortRequested` six times -- declare, set, and handle -- against four here, all declare or set."),
        ("Two permanent consequences", "The caller's `&'static mut [u8]` stays in the driver's `TakeCell`. And `transmit_buffer` (`:911`) proceeds only from `Idle`, so with the state stuck at `AbortRequested` every later transmit answers `Err(BUSY)` for the life of the board. `receive_buffer` behaves the same way."),
        ("Reachable, not theoretical", "`boards/lpc55s69-evk/src/main.rs` builds a `UartMuxComponent` and puts three clients on it: the userspace console at `:347`, a debug writer at `:356` and a process console at `:374`. `MuxUart::start_receive` calls `receive_abort` whenever it restarts a read to recalculate the minimum length, which is routine with more than one receiving client. The comment at `virtual_uart.rs:288` says it aborts *so we can use the `received_buffer()` handler to recalculate*, and on this driver that handler was never reached."),
        ("The fix is the driver's own completion sequence", "Return to `Idle`, then hand the buffer back with `Err(CANCEL)` and however much was transferred. That is what `handle_interrupt` already does to finish a normal transfer, in the same order, and sam4l's `abort_tx` resolves the same situation the same way. `AbortRequested` is removed from both enums: nothing constructs it now, and leaving it in fails the build -- restoring the variant and running `cargo clippy -p lpc55s6x -- -D warnings` exits 101 with `error: variant AbortRequested is never constructed`, and that flag is what `ci-job-clippy` runs at `Makefile:407`."),
        ("What was run, and what was not", "lpc55s69-evk builds, and text, data and bss are unchanged at 65580, 0 and 25892 -- measured by stashing the change and rebuilding, not assumed. `cargo clippy -p lpc55s6x -- -D warnings`, `cargo fmt --check` and Tock's license checker pass. **Nothing ran on hardware**: no lpc55s69-evk is available here, so this rests on the code and on the in-tree driver that already handles the case this way."),
    ],
    ...
    "tock:rp2350-stale-nvic-pending": [
        ...
        ("The pending bit is a latch, not a live interrupt", "Read over SWD while the board sat in its own panic handler: **NVIC ISPR bit 14 is set while the USB block's MAIN_CTRL, INTR, INTE and INTS are all zero.** The controller is not enabled, no raw condition is set, and with INTE at zero its output cannot be asserted at all, and the whole 256-byte block is byte-for-byte identical to the same registers read on a boot that did not panic. A pending bit set beside a quiet source was latched earlier and never cleared. That single pair of numbers separates the two candidate stories: a live interrupt would need servicing or masking, a stale latch needs clearing once the source is gone."),
        ("Why the existing clear cannot do it", "`Chip::init()` already runs `clear_all_pending()`. It runs *before* `reset_all_except`, and what the bootrom hands over was measured rather than assumed -- halted over SWD while it was enumerated as `2e8a:000f`, the USB block has MAIN_CTRL set, address 0x24 assigned by a host, frames arriving and INTE at 0x113f0, **and the bootrom has enabled USBCTRL_IRQ in the NVIC itself**. A source asserting at that moment re-latches as fast as the clear drops it; a level-sensitive line cannot be cleared out from under. The peripheral reset afterwards silences the source and leaves the latched bit behind. `disable_all()` does not save it either: `next_pending_with_mask` reads the pending register, not the enable register. The original two-in-nine needs no mechanism, only the condition: both sightings were resets taken out of a *live* session and both panicked, while the rest either had no bootrom involved or had let the board self-boot out of it first, which ends the session. Over the runs that met the condition it is 2 of 2; over the seven that did not, 0 of 7."),
        ("Reproducing it with nobody in the room", "Both original sightings followed a bootrom USB session, which meant holding BOOTSEL -- so no rate could be measured and no fix could be tested. **Erasing the first flash sector takes the `IMAGE_DEF` with it, so the bootrom finds no image and falls back to its own USB device**; wait for `2e8a:000f` to enumerate, then reprogram and reset from the debugger while that session is live. The panic went from twice in nine resets with no known trigger to **thirteen in thirteen on demand** -- every run of this variant on an unpatched kernel, openocd ending the session. **Both debuggers produce it, but not at the same rate**: ending the bootrom session with probe-rs instead panicked once in six. The obvious explanation was session length, since that path takes 96 s an iteration against 18 s -- **and it is wrong.** Holding the session open a further 78 s before letting openocd reprogram makes the iteration 96 s too, and it still panics 6 in 6. Same tool, same commands, matched duration, rate unmoved: **length is not the variable, the tool is**, and what differs between them has not been isolated. The earlier reading -- that reset method is not a variable at all -- was resting on one occurrence per tool and is dead either way."),
    ...
    "tock:rp2-uart-abort-fix": [
        ("What it fixes", "Three defects in one path. In `chips/rp2040/src/uart.rs`, `chips/rp2350/src/uart.rs` and `chips/sifive/src/uart.rs` an aborted receive calls the client back before the driver returns to Idle; `capsules/core/src/virtualizers/virtual_uart.rs` restarts from inside that callback, is refused, and ends every client's receive. Separately, a failed `start_receive` handed the caller the mux's buffer instead of its own, and the teardown dropped the buffer of any device that was not in the Receiving state."),
    ...
    "libtock-rs:main": [
        ("What it is", "The fork's trunk and default branch since 2026-09-14. `master` was 2 behind upstream and is now fast-forwarded to it and tracking it directly; `main` is `master` plus merges of doom/frame-probe, async/alarm and bench/reclaim-leak. 89 commits ahead of upstream/master."),
        ("What the restructure removed", "origin went from 40 branches to 9. Thirty were byte-identical mirrors of upstream inherited at fork time -- bors-actions, rust_start, ufmt-*, msrv, trying, staging, 2019 to 2025 vintage. kit-examples and bench/reclaim-leak were folded in and deleted, each verified an ancestor of main first. Zero open pull requests had a head in this fork, checked before anything was deleted."),
        ("Why five branches were NOT folded", "unittest-fakes, platform-pico2, pico2-platform, stepper and async/alarm are rooted exactly on master's tip -- minimal one-concern diffs, which is the entire work product. unittest-fakes was extracted from async/alarm on purpose and drops 36 lines to stay minimal. Containment is the wrong test: deleting them keeps the content and destroys the extraction. A ref costs four lines in `git branch -r`; re-extracting one costs hours."),
        ("Three failures it exposed", "None caused by the merge -- doom/frame-probe fails the same way alone, because the gate had never run against a fresh checkout. kit_wad_xip include_bytes! a gitignored 1.8 MB generated WAD, so it built only where the converter had been run. kit_doom_frame's 320x200 buffer overflowed nrf52 by 67584 bytes -- `make examples` builds 2 platforms while the table lists 25. And doom.rs had never passed rustfmt."),
        ("Gate", "make examples and make test both pass on main."),
    ],
    "libtock-rs:doom/frame-probe": [
        ("What it is", "Doom, playable on a Pico 2 W: full screen at 480x320, 9.0 fps, joystick, both buttons and the exit lever. 78 commits, and it fully contains hw/pico2w-async, pico2-platform and stepper."),
        ("Where the RAM came from", "266 KB recovered. The largest single piece was a guess rather than a measurement -- `stack_size!` was set to 32768 and Doom uses 1984."),
        ("The finding that outlived Doom", "The missing half of the SPI bus was the kernel, not the bus. `copy_to_slice_or_err` copied process buffers a byte at a time, about 19 cycles each, ~46 ms per 307200-byte frame. Fixed in tock `846a29a77`; it benefits every Tock app moving bulk data through a process buffer. Display went from 41% of the panel at 12.5 fps to 100% at 9.0, write rate 2.59 to 4.66 MB/s."),
        ("Licensing", "Nothing GPL is vendored here. The C is GPL-2.0-only and lives in via-balaena/doomgeneric, a fork of ozkl/doomgeneric. The Rust is original and stays Apache-2.0 OR MIT; --features doom is off by default and is what links the GPL code. LICENSE-DOOM records that the resulting binary is GPL-2.0-only and why the MIT half of the disjunction is what makes that lawful."),
    ],
    "libtock-rs:stepper": [
...
FINDINGS = [
    ...
    (5160, "5160", "a flashing route with no ELF to get wrong",
     ...
     "run end to end -- assembled by tockloader, loaded three ways (SWD, the "
     "bootloader, probe-rs), with two applications co-resident and live in the "
     "kernel's own process list. It cannot hit the defect because it never "
     "opens an ELF. He then asked for it in the shape esp32-c3-devkitM-1 "
     "already uses, which retired the cost this page had led with: make no "
     "longer takes an APP at all. Four costs remain, and the tool swap on the "
    ...
    (5165, "5165", "a pending interrupt survives the reset that silenced it",
     ...
     "while the USB block reads byte-for-byte identical to a boot that did not "
     "panic. The "
