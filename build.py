...
FACTS = {
    ...
    "tock:capability-compile-fail": [
        ...
        ("The mechanisms differ, and the documentation now says so", "`create_typed_capability` and `define_capability_type` carry an internal `allow(unsafe_code)` that collides with a forbid, giving `error[E0453]: allow(unsafe_code) incompatible with previous forbid`. `mint_defined_capability` is refused because the caller's own `unsafe` block is, which is a different guarantee -- worth naming rather than implying, since only the first kind survives someone removing the internal allow."),
