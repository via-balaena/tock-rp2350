...
INTENT = {
    ...
    "tock:library-floor-forbid": ("upstream", "AGENTS.md says new code in `capsules/`, `chips/` and `libraries/` may not use `unsafe` at all. Of the five crates under `libraries/`, exactly one gives that to the compiler. `enum_primitive` contains no `unsafe` and now forbids it; `tock-cells` confines its six sites to `map_cell` and denies the lint everywhere else. Two files, +10, attributes only.", "Ready, not sent. Level 1 of the floor plan, on top of level 0's `StaticRef::at`. The rule already held in both crates -- nothing enforced it, which is the same gap the main audit measured at 0 against 1,226. Proven by three mutations, each rejected, including removing the `map_cell` allow, because a lint that never rejects anything has never been tested. `raspberry_pi_pico_2` builds byte-identical with and without, 262144 bytes to the same sha256."),
...
FACTS = {
    ...
    "tock:library-floor-forbid": [
        ...
        ("Three mutations, each rejected", "An `unsafe` block appended to `take_cell.rs` fails with `error: usage of an unsafe block`. The same block in `enum_primitive/src/cast.rs` fails. Removing `#[allow(unsafe_code)]` from the `map_cell` declaration also fails, and that is the one that matters: it shows the `deny` is live rather than inert. Each was reverted and both crates rebuilt clean afterwards."),
        ("What was run", "`make ci-job-libraries` exits 0, which covers all four library crates it names and runs tock-cells' 32 tests. `make -C boards/raspberry_pi_pico_2` exits 0. The binary is byte-identical with and without the change -- 262144 bytes, sha256 `809d28d982e1b839...`, compared by stashing the diff and rebuilding rather than by reasoning that attributes cannot emit code. `cargo fmt --check` and Tock's license checker are both clean."),
