...
INTENT = {
    ...
    "tock:rp2-local-board": ("upstream", "Replaces the ELF splice on all four RP2 boards with a flash image built by `tockloader local-board`: the kernel binary at the base of flash, applications installed from `.tab` archives, and the flat image handed to picotool, openocd or probe-rs. elf2uf2-rs leaves the tree. Two commits, eight files, +180 -73.", "**Open as #5160**, head `6d902052f`. This is the successor bradjc asked for on #5156 and the answer given there. It supersedes that PR's `objcopy -R .stack` on the boards it touches, and is based on `e2ec78ff5` rather than on that branch, so either can merge first. Touches the same two README lines as #5158. The Pico 2 half ran on silicon over all three load paths -- SWD, the bootloader and probe-rs -- and placed two applications at once, which the splice route cannot do at all; **no RP2040 was ever booted** -- there is no such board here and mainline QEMU has no machine for it."),
...
WORK = {
    ...
    "rp2350: build the flashed image with tockloader, not by splicing an ELF": {
        ...
        "verify": "silicon",
        "note": "PR #5160, first commit. Removes the conditions for #4770 rather than patching it: no ELF is opened, so no program header is recomputed. Run on a Pico 2 W every way the rules offer -- SWD, the bootloader and probe-rs -- and it places two applications at once, which the splice cannot do at all.",
    },
    "rp2040: the same image route, and one UF2 converter instead of two": {
        "short": "Same route on RP2040, picotool only",
        "verify": "build",
        "note": "PR #5160, second commit. elf2uf2-rs cannot read a flat image, so these three boards move to picotool and elf2uf2-rs leaves the tree. Measured rather than assumed: from the same kernel ELF the two converters agree on all 102,400 addresses with zero disagreements. No RP2040 was booted -- there is no such board here and mainline QEMU has no machine for one.",
    },
