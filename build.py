...
FACTS = {
    ...
    "tock:rp2-local-board": [
        ...
        ("On silicon, three load paths", "All three ways of putting the image on the board were run against a Pico 2 W over a Debug Probe. **SWD**: `make flash-openocd` by its own name, with openocd executing on the machine holding the probe -- it programs, verifies, resets, and the kernel's own process console lists the application as live. **The bootloader**: `make flash`'s UF2, 549,376 bytes in 1,073 blocks, copied onto the mounted RP2350 volume; the board left BOOTSEL by itself and booted. **probe-rs**: `probe-rs download --binary-format bin --base-address` then `probe-rs reset`, finished in 17.69s -- the same flag form the rule uses, with `--chip RP235x` because that is the board here. And two applications at once, which the splice route cannot do at all: **one** `tockloader install --local-board` with both archives -- `leak_claim` 4,096 bytes at 0x10040000, `leak_observe` 8,192 bytes at 0x10041000, a 274,433-byte file -- both listed by the kernel. **The empty case too**: with the application region erased and nothing installed, the dense 262,144-byte image boots and `list` returns its header row and nothing under it."),
        ("The stale slot, and the one byte that closes it", "The kernel's loader stops at the first invalid TBF header rather than scanning on -- checked by placing a valid application one sector further along and watching it never appear. So only the slot immediately after the last application can resurrect an old one. tockloader writes a zero byte there; splicing writes nothing. Two images differing by that byte alone were flashed over an identical board state: without it the kernel loaded an application that was not in the image it was given. That is a defect in the route in the tree, not a caveat about this one. **And the byte does the work, not the sector erase that normally accompanies it** -- `flash fillb <slot> 0x00 1` writes it with no erase at all, dumps either side differ at exactly one position (the TBF version field, 02 to 00), and the ghost is gone on the next reset."),
    ...
    "tock:stepper-capsule": [
        ...
        ("Why it is a capsule rather than an application", "Nothing releases a GPIO pin when the process that set it dies. `doc/syscalls/00004_gpio.md:12` describes the driver as exporting \"hardware-like\" control, so having no ownership to unwind is the documented intent rather than an oversight. A motor driven straight through it keeps a coil energised for the life of the board if its process faults mid-step. That policy is device knowledge, so it belongs where it can be enforced. The liveness check rides on the alarm a stepper already needs: the owner's grant is entered before every step, and a dead owner de-energises and stops within a single step interval. Its cost in code size was not measured."),
...
QUEUE_DEPS = {
    ...
    "tock:rp2-local-board": [
        (5156, "Supersedes it. This deletes the `-R .stack` lines that pull "
               ...
               "**the only artefact in the queue with RP2040 hardware behind "
               "it** -- the reporter ran it on his own board, and this one has "
               "never booted an RP2040 and cannot. Its two approvals were "
               "**dismissed on 2026-09-10** by a force-push correcting the "
               "Makefile comments and the commit message, so it is waiting on "
               "bradjc and ppannuto again."),
    ],
    "libtock-rs:pico2-platform": [
