...
INTENT = {
    ...
    "tock:rp2-make-program-fix": ("upstream", "Fixes #4770: `make program` cannot flash an application on any RP2 board, because splicing one in gives the (NOLOAD) `.stack` segment file content for SRAM and both UF2 converters refuse the result. One `objcopy -R .stack` per rule, five rules, four boards.", "#5154 was opened and closed the same day, for having a placeholder description rather than for the change. It is now the strongest thing here: the reporter ran the fix on his own RP2040, and `program-openocd` was A/B'd on an RP2350. Four Makefiles, twenty-one added lines, no Rust."),
